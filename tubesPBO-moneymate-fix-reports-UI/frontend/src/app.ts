...
class MoneyMateUI {
    ...
    private async loadCategories(): Promise<void> {
        try {
            const raw = await this.api.getCategories();

            // NORMALISASI DATA DARI BACKEND
            this.categories = {
                income: Object.entries(raw.income || {}).map(([key, value]: [string, any]) => ({
                    name: key,
                    displayName: typeof value === 'string' ? value : (value.displayName || key)
                })),
                expense: Object.entries(raw.expense || {}).map(([key, value]: [string, any]) => ({
                    name: key,
                    displayName: typeof value === 'string' ? value : (value.displayName || key)
                }))
            };
    ...
    private exportReportCsv(): void {
        ...
        // Header CSV dengan informasi laporan
        ...
        csv += `Total Pemasukan,${this.formatNumber(report.totalIncome)}\n`;
        csv += `Total Pengeluaran,${this.formatNumber(report.totalExpense)}\n`;
        csv += `Saldo Bersih,${this.formatNumber(report.balance)}\n`;
        csv += '\n';
        
        // Pemasukan per Kategori
        csv += '=== PEMASUKAN PER KATEGORI ===\n';
        csv += 'Kategori,Jumlah (Rp),Persentase (%)\n';
        Object.entries(report.incomeByCategory || {}).forEach(([category, amount]: [string, any]) => {
            ...
            csv += `${this.getCategoryDisplayName(category)},${this.formatNumber(amount)},${percentage}\n`;
        });
        csv += `TOTAL PEMASUKAN,${this.formatNumber(report.totalIncome)},100.00\n`;
        csv += '\n';
        
        // Pengeluaran per Kategori
        csv += '=== PENGELUARAN PER KATEGORI ===\n';
        csv += 'Kategori,Jumlah (Rp),Persentase (%)\n';
        ...
        csv += `TOTAL PENGELUARAN,${this.formatNumber(report.totalExpense)},100.00\n`;
        csv += '\n';
        
        // Footer
        csv += '=== INFO ===\n';
        csv += `Generated by MoneyMate\n`;
        csv += `© ${new Date().getFullYear()} MoneyMate - Personal Finance Manager\n`;
    ...
    private exportReportTxt(): void {
        ...
        // Ringkasan Keuangan
        txt += '📊 RINGKASAN KEUANGAN\n';
        txt += subLine + '\n';
        txt += `  Total Pemasukan        : Rp ${this.formatNumber(report.totalIncome).padStart(20)}\n`;
        txt += `  Total Pengeluaran      : Rp ${this.formatNumber(report.totalExpense).padStart(20)}\n`;
        txt += `  ${subLine.substring(0, 30)}\n`;
        txt += `  Saldo Bersih           : Rp ${this.formatNumber(report.balance).padStart(20)}\n`;
        txt += '\n\n';
        
        // Pemasukan per Kategori
        txt += '💰 PEMASUKAN PER KATEGORI\n';
        txt += subLine + '\n';
        txt += '  Kategori                    Jumlah                  Persentase\n';
        txt += subLine + '\n';
        Object.entries(report.incomeByCategory || {}).forEach(([category, amount]: [string, any]) => {
            const percentage = report.totalIncome > 0 ? ((amount / report.totalIncome) * 100).toFixed(2) : '0.00';
            const catName = this.getCategoryDisplayName(category).padEnd(22);
            const amtStr = ('Rp ' + this.formatNumber(amount)).padStart(25);
            const pctStr = (percentage + '%').padStart(12);
            txt += `  ${catName} ${amtStr} ${pctStr}\n`;
        });
        txt += subLine + '\n';
        txt += `  ${'TOTAL'.padEnd(22)} ${('Rp ' + this.formatNumber(report.totalIncome)).padStart(25)} ${'100.00%'.padStart(12)}\n`;
        txt += '\n\n';
        
        // Pengeluaran per Kategori
        txt += '💸 PENGELUARAN PER KATEGORI\n';
        txt += subLine + '\n';
        txt += '  Kategori                    Jumlah                  Persentase\n';
        txt += subLine + '\n';
        Object.entries(report.expenseByCategory || {}).forEach(([category, amount]: [string, any]) => {
        ...
        txt += subLine + '\n';
        txt += `  ${'TOTAL'.padEnd(22)} ${('Rp ' + this.formatNumber(report.totalExpense)).padStart(25)} ${'100.00%'.padStart(12)}\n`;
        txt += '\n\n';
        
        // Analisis
        txt += '📈 ANALISIS\n';
        txt += subLine + '\n';
        const savingRate = report.totalIncome > 0 ? 
            ((report.balance / report.totalIncome) * 100).toFixed(2) : '0.00';
        txt += `  Tingkat Tabungan       : ${savingRate}%\n`;
        txt += `  Status                 : ${report.balance >= 0 ? 'SURPLUS ✓' : 'DEFISIT ✗'}\n`;
        
        if (report.totalIncome > 0) {
            txt += `\n  Rekomendasi:\n`;
            if (parseFloat(savingRate) >= 20) {
                txt += `  • Bagus! Anda menabung ${savingRate}% dari pemasukan.\n`;
            } else if (parseFloat(savingRate) >= 10) {
                txt += `  • Cukup baik, coba tingkatkan tabungan ke minimal 20%.\n`;
            } else {
                txt += `  • Perhatikan pengeluaran! Tingkatkan tabungan Anda.\n`;
            }
        }
        txt += '\n\n';
        ...
        txt += `  © ${new Date().getFullYear()} MoneyMate. All rights reserved.\n`;
        txt += line + '\n';
